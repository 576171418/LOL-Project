...
internal let FSCCycleCellID = "FSCCycleCell"

public typealias FSCCycleViewCustomCellHandle = (_ collectionView: UICollectionView, _ indexPath: IndexPath, _ picture: String) -> UICollectionViewCell

//滚动方向
public enum FSCCyclePictureViewRollingDirection: Int {
    case top
    case left
    case bottom
    case right
...
class FSCCyclyView: UIView {
    ...
    //default is 2.0，如果小于0.5不自动播放
    open var autoScrollDelay: TimeInterval = 2
    
    //滚动方向
    open var direction: FSCCyclePictureViewRollingDirection = .left {
        willSet {
            switch newValue {
            case .left, .top:
                self.pageControl.currentPage = 0
    ...
    //pageControl的对齐方式
    open var pageControlStyle: PageControlStyle = .center {
        willSet {
            guard self.titles.count > 0 else { return }
            let poinSize: CGSize = self.pageControl.size(forNumberOfPages: self.titles.count)
            let page_x: CGFloat = (self.pageControl.bounds.size.width - poinSize.width) / 2
    ...
    //自定义cell的回调
    open var customCellHandle: FSCCycleViewCustomCellHandle?
    
    //如果需要自定义Cell需调用下面方法
    open func register(_ cellClasss: [Swift.AnyClass?], identifiers: [String], customCellHandle: @escaping FSCCycleViewCustomCellHandle) {
    ...
    open func register(_ nibs: [UINib?], identifiers: [String], customCellHandle: @escaping FSCCycleViewCustomCellHandle) {
        self.customCellHandle = customCellHandle
        for (index, identifier) in identifiers.enumerated() {
    ...
    open lazy var pageControl: JPageControl = {
        let pageControl = JPageControl(frame: CGRect(x: 0, y: self.frame.size.height - 20, width: self.frame.size.width, height: 20))
    ...
    internal lazy var collectionView: UICollectionView = {
        ...
        let collecctionView: UICollectionView = UICollectionView(frame: CGRect(x: 0, y: 0, width: self.frame.size.width, height: self.frame.size.height), collectionViewLayout: layout)
        collecctionView.showsHorizontalScrollIndicator = false
        collecctionView.showsVerticalScrollIndicator = false
        collecctionView.register(FSCCyclePictureViewCell.self, forCellWithReuseIdentifier: FSCCycleCellID)
        collecctionView.backgroundColor = UIColor.clear
        collecctionView.isPagingEnabled = true
        collecctionView.bounces = false
        collecctionView.dataSource = self
        collecctionView.delegate = self
        self.addSubview(collecctionView)
...
extension FSCCyclyView: UIScrollViewDelegate {
...
extension FSCCyclyView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let customCellHandle = self.customCellHandle {
            ...
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FSCCycleCellID, for: indexPath) as! FSCCyclePictureViewCell
...
class FSCCyclePictureViewCell: UICollectionViewCell {
